...
FUNC smapi_formal_function_name `(' host`'ifelse(myClassName(),`CheckAuthentication', `', ` target_identifier')`'dnl
opt_ctor_param_list paramarray `)' `$'`'`['
        smapi -to @@paramarray @@host smapi_formal_function_name`'dnl
...
include(`pigiron_copyright_ublucomment.txt')
changecom(`/*')dnl
`# Autogenerated' date_string()`# by ublu_pigfunc.m4 Copyright *C* 2016 Jack J. Woehr'
`# Part of the PigIron Project http://pigiron.sourceforge.net'
changecom(`#')dnl
pop_divert()dnl
')
